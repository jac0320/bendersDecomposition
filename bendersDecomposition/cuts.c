...
int formSingleCut(probType **prob, cellType *cell) {
	...
	cut = newCut(cell->omega->cnt, cell->k, prob[1]->num->cntCcols);
	if ( cut == NULL ) {
	...
	istar = addCut(cell->master->lp, cell->cuts, prob[0]->num->rows, prob[0]->num->cols, cell->maxCuts, 0, cut);
	if ( istar < 0 ) {
...
int addCut(LPptr lp, cutsType *cuts, int numRows, int numCols, int maxCuts, int etaIndex, oneCut *cut) {
	...
	if ( !(indices = (intvec) arr_alloc(numCols+1, int)) )
		errMsg("allocation", "addCut", "indices", 0);
	for ( n = 1; n <= numCols; n++ )
		indices[n] = n-1;
	indices[0] = numCols + etaIndex;
	...
	if ( addRow(lp, numCols+1, cut->alpha, cut->sense, 0, indices, cut->beta) ) {
